...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest1) {
  ...
  std::thread t1([&] {
    ...
    std::cout << "t1 get rid0 false" << std::endl;

    EXPECT_EQ(TransactionState::ABORTED, txn1->GetState());
    txn_mgr.Abort(txn1);
    std::cout << "t1 abort" << std::endl;
  });
...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest2) {
  ...
  std::thread t0([&] {
    ...
    // std::cout << "t0 get X lock" << std::endl;
  ...
  std::thread t1([&] {
    ...
    // std::cout << "t1 get X lock" << std::endl;
