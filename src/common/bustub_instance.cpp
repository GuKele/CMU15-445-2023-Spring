...
auto BustubInstance::ExecuteSqlTxn(const std::string &sql, ResultWriter &writer, Transaction *txn,
   ...
   *
   * 2.然后再通过Binder会将标识符绑定到数据库实体上（表名、列名），然后把这个 AST 改写成
   * BusTub可以理解的一个更高级的BusTub AST。
   *
   ...
   * 然后通过 optimizer 框架做很多步优化产生 physical plan node。但是 BusTub 只是个教学项目，所以我们只有 physical plan
   * node。 Planner 会直接把 join plan 成 NestedLoopJoin，在 optimizer 里面改写成 HashJoin 或者 NestedIndexJoin。
   *
   * 4. BusTub optimizer 是一个 rule-based 优化器。我们将不同的 rule 按顺序应用到当前的执行计划上，产生最终的执行计划。
   * 每一条 rule由开发者手动实现，我们并不提供一个通用的改写框架。目前大部分 rule
   * 实现的方式都是bottom-up，自下而上改写整个 query plan。
   */
