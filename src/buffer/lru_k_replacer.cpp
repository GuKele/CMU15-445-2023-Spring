...
LRUKNode::LRUKNode(frame_id_t frame_id) 
  : fid_(frame_id)
{ }

LRUKReplacer::LRUKReplacer(size_t num_frames, size_t k) 
  : max_frame_capacity_(num_frames), 
    k_(k) 
{ }
...
{
  assert(frame_id < static_cast<int32_t>(max_frame_capacity_));
  ...
  if(map_it == nodes_.end()) 
  {
    if(nodes_.size() == max_frame_capacity_ && !EvictImpl(nullptr)) {
      ...
      nodes_[frame_id] = std::make_pair(--non_k_nodes_fifo_.end(), LRUKNode(frame_id));
    }
  } 
  else  // 本来就有
  { 
...
auto LRUKReplacer::EvictImpl(frame_id_t *frame_id) -> bool {
  ...
  for (auto it = non_k_nodes_fifo_.rbegin(); it != non_k_nodes_fifo_.rend(); ++it) {
    auto node = nodes_[*it].second;
    if (node.GetIsEvictable()) {
      *frame_id = node.GetFrameId();
      RemoveImpl(*frame_id);
